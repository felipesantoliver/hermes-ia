...
(function () {
  ...
  /* ---------- Modal customizado: renomear conversa ---------- */
  const renameModal = document.getElementById('rename-chat-modal');
  const renameInput = document.getElementById('rename-chat-input');
  const renameCancelBtn = document.getElementById('cancel-rename-chat-modal');
  const renameConfirmBtn = document.getElementById('confirm-rename-chat-modal');
  ...
  function openRenameModal(chat) {
    ...
    const onConfirm = () => {
      ...
      if (newTitle) patchChat(chat.id, { title: newTitle });
      cleanup();
    };
  ...
  /* ---------- Modal customizado: confirmação genérica (ex. excluir) ---------- */
  const confirmModal = document.getElementById('confirm-action-modal');
  const confirmTitleEl = document.getElementById('confirm-action-title');
  const confirmMessageEl = document.getElementById('confirm-action-message');
  const confirmCancelBtn = document.getElementById('cancel-confirm-action-modal');
  const confirmConfirmBtn = document.getElementById('confirm-confirm-action-modal');
  ...
  async function deleteChat(chat) {
    const confirmed = await askConfirmation({
      ...
      message: `Excluir a conversa "${chat.title}"? Essa ação não pode ser desfeita.`,
